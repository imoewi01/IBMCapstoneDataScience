...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        new_df = spacex_df[mask]
        fig = px.pie(new_df, values='class',
        names='Mission Outcome',
        title='Total Success Launches for '+str(entered_site))#+new_df['Launch Site'][0])
        return fig
# TASK 4:
...
def get_scatter_plot(entered_site, payload_mass_vals):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        mask = filtered_df['Payload Mass (kg)']>payload_mass_vals[0]
        filtered_df[mask]
        mask = filtered_df['Payload Mass (kg)']<payload_mass_vals[1]
